...
public class Disabler extends Module {
    ...
    public final ModeProperty modifyMode = new ModeProperty("modify-mode", 0, new String[]{
            "ConvertNull", "Spoof", "Zero", "SpoofZero", "Negative", "OffsetYaw"
    });
    ...
    private double lastSpeed2d = 0.0D;
    ...
    public void onPacket(PacketEvent event) {
        ...
        if (this.noRotationDisabler.getValue() && packet instanceof C03PacketPlayer) {
            C03PacketPlayer c03 = (C03PacketPlayer) packet;
            switch (this.modifyMode.getValue()) {
                case 0:
        ...
        if (this.betaVerus.getValue()) {
            if (!(packet instanceof C0FPacketConfirmTransaction)) {
                if (packet instanceof C03PacketPlayer) {
                    if (mc.thePlayer.ticksExisted % this.betaVerusFlagDelay.getValue() == 0
                            && mc.thePlayer.ticksExisted > this.betaVerusFlagDelay.getValue() + 1
                            && !this.betaVerusModified) {
                ...
                if (this.betaVerusPacketBuffer.size() > this.betaVerusBufferSize.getValue()) {
                    ...
                    Packet<INetHandlerPlayClient> bufferedPacket = this.betaVerusPacketBuffer.poll();
                    int repeatTimes = this.isOnCombat
                            ? this.betaVerusRepeatTimesFighting.getValue()
                            : this.betaVerusRepeatTimes.getValue();

                    for (int i = 0; i < repeatTimes; ++i) {
                        PacketUtil.sendPacketNoEvent(bufferedPacket);
                    }
                }
        ...
        if (this.verusExperimental.getValue()) {
            if (this.verusExpVoidTP.getValue() && packet instanceof C03PacketPlayer) {
                if (mc.thePlayer.ticksExisted > 20
                        ...
                        && this.lastVoidTP + this.verusExpVoidTPDelay.getValue() < System.currentTimeMillis()) {
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (this.vulcanScaffold.getValue()
                ...
                && this.isMoving()
                && mc.thePlayer.ticksExisted % this.vulcanPacketTick.getValue() == 0) {
        ...
        if (this.matrixDisabler.getValue() && this.matrixTA.getValue()) {
            ...
            if (currentSpeed > 0.001D) {
                ...
                if (!mc.thePlayer.onGround && speedDiff < 1.0E-4D) {
                    MoveUtil.setSpeed(currentSpeed * (0.99999999999D - Math.random() * 1.0E-7D), MoveUtil.getMoveYaw());
                }
            }
            ...
            if (this.lastC03timer.hasTimeElapsed(4000L) && MoveUtil.getSpeed() < 0.001D) {
                this.lastC03timer.reset();
                this.shouldDelayMatrix = (double) this.randomLong * 1.3D < (double) this.c0fCount;
                this.randomLong = 0L;
                this.c0fCount = 0;
    ...
    private void handleMatrixPacket(PacketEvent event, Packet<?> packet) {
        if (packet instanceof C0FPacketConfirmTransaction && this.matrixT.getValue() == 1) {
            ...
            if (!event.isCancelled()) {
                ...
                event.setCancelled(true);
                long secondTime = !this.c0fs.isEmpty() ? this.c0fs.getLast().getRequiredMs() : 200L;
                if (secondTime - firstTime >= 20L) {
                    this.c0fs.pollLast();
                }
            }
        ...
        if (packet instanceof C03PacketPlayer && this.matrixT.getValue() == 1) {
            ...
            if (this.savedAbusePacket < 5 && !this.c0fs.isEmpty()) {
                ...
                if (!this.c0fs.isEmpty()) {
                    long secondTime = this.c0fs.getFirst().getRequiredMs();
                    if (secondTime - firstTime < 20L) {
                        PacketUtil.sendPacketNoEvent(this.c0fs.pollFirst().getPacket());
                    }
                }
    ...
    private void debugMessage(String msg) {
        if (this.chatDebug.getValue() && mc.thePlayer != null) {
